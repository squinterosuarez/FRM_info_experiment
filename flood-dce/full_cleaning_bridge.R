## full_cleaning_bridge.R — Qualtrics full-sample export -> apollo-ready long database.
...
## to carry the effects-coded A_*/B_* attribute columns. Writes data/full_database.rds.
...
d <- read.csv("data/full_results.csv", stringsAsFactors = FALSE, check.names = FALSE)
#t2col <- grep("different from what you had expected", names(d), ignore.case = TRUE)
t2col <- grep("T2", names(d), ignore.case = TRUE)
...
saveRDS(db, "data/full_database.rds")

cat(sprintf("wrote data/full_database.rds: %d obs, %d respondents\n", nrow(db), length(unique(db$ID))))
