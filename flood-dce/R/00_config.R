...
ATTR <- list(a1_levels=1:4, a2_levels=1:3, a3_levels=1:3,
             a4_levels=c(1,3), cost_levels=c(75,150,300),
             sq=list(a1=3,a2=1,a3=2,a4=2,cost=0))
EC <- list(a1_base=4, a2_base=3, a3_base=3, a4_base=3)
...
RP <- c("asc","a1e1","a1e2","a1e3","a2e1","a2e2","a3e1","a3e2","a4e1")

TRUE_mu <- c(asc=-0.40, a1e1=0.50,a1e2=0.00,a1e3=0.40, a2e1=0.30,a2e2=0.00,
             a3e1=-0.20,a3e2=0.40, a4e1=0.50, cost=-0.80)
## being informed, no surprise (correct prior): near zero
TRUE_dt <- c(asc=-0.05, a1e1=0,a1e2=0,a1e3=0, a2e1=0,a2e2=0, a3e1=0,a3e2=0, a4e1=0, cost=0)
## per upward step (learned higher than expected): "protect me, don't charge me"
TRUE_up <- c(asc=-0.15, a1e1=-0.10,a1e2=0.20,a1e3=0.00, a2e1=0.15,a2e2=0.00,
             a3e1=0.20,a3e2=0.10, a4e1=0.15, cost=0)
## per downward step (learned lower than expected): SQ / opt-in / let risky pay
TRUE_dn <- c(asc=0.20, a1e1=-0.20,a1e2=-0.05,a1e3=0.00, a2e1=-0.15,a2e2=0.00,
...
TRUE_np_asc <- -0.10
TRUE_sd <- c(asc=1.0, a1e1=0.5,a1e2=0.5,a1e3=0.4, a2e1=0.4,a2e2=0.3, a3e1=0.4,a3e2=0.4, a4e1=0.4)
...
TRUE_alpha <- c(asc=-0.05, a1e1=0,a1e2=0,a1e3=0, a2e1=0,a2e2=0, a3e1=0,a3e2=0, a4e1=0, cost=0)
TRUE_beta  <- c(asc= 0.00, a1e1=0,a1e2=0,a1e3=0, a2e1=0,a2e2=0, a3e1=0,a3e2=0, a4e1=0, cost=0)
TRUE_gamma <- c(asc=-0.10, a1e1=-0.10,a1e2=0.20,a1e3=0.00, a2e1=0.15,a2e2=0.00,
                a3e1= 0.20,a3e2=0.10, a4e1=0.15, cost=0)

## ---- bundles on (A1,A2,A3); welfare == SQ config == reference ----
## Two variants of the club good are reported in parallel:
##   club       (A2.3 local-only): pure club good - beneficiary group pays
##   club_mixed (A2.2 national+local): more realistic funding mix
## `club` (A2.3) is the primary; club_mixed is reported as a parallel variant.
BUNDLES <- list(
  public     = c(a1=1, a2=1, a3=1),   # all / national / flat
  welfare    = c(a1=3, a2=1, a3=2),   # SQ config (reference; WTP defined = 0)
  club       = c(a1=2, a2=3, a3=1),   # high-risk area / LOCAL ONLY  / flat
  club_mixed = c(a1=2, a2=2, a3=1),   # high-risk area / national+local / flat
  private    = c(a1=4, a2=3, a3=3)    # opt-in / local-only / risk-priced
)
BUNDLE_REF <- "welfare"
PAIRWISE <- list(c("public","club"),       c("public","private"),       c("club","private"),
                 c("public","club_mixed"), c("club_mixed","private"),   c("club","club_mixed"))
