## pilot_h2o.R — PAP hypothesis H2o (directional separation) on the pilot.
##
...
##
## Pilot is too small for random coefficients, AND per-cell own-cost fits are
## unstable (over-treated cost ~ -0.19, indistinguishable from 0 -> WTP ratio
## blows up). So: pooled fixed-coef MNL on the under u over respondents with a
## COMMON cost coefficient (the main spec fixes cost anyway) and cell-specific
...
cat("p_onesided is for the PAP-registered direction of each row",
    "(under & SEP: >0 ; over: <0).\n")
cat("\nCAVEAT: underestimator cell is 6 control / 5 treated. This is a direction\n",
    "check, not a powered test; treat the CI as indicative only.\n", sep="")
cat("\nPILOT_H2O_DONE\n")
